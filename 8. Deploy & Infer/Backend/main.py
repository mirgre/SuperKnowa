...
with open(
    "/workspaces/SuperKnowa/8. Deploy & Infer/Backend/model_configuration.json", "r"
) as f:
    model_configurations = json.load(f)["models"]
...
def chat():
    ...
    model_output = model_request.process_request_using_wd(
        question, authenticator, info_collect
    )
...
def chat_single_model(retriever):
    ...
    model_request = ModelRequestService(model_configurations)
    answer, source = model_request.process_request_using_single_model(
        question, retriever, info_collect
    )
...
def chat_with_given_context():
    ...
    model_request = ModelRequestService(model_configurations)
    model_output = model_request.process_request_with_given_context(
        question, context=context, info_collect=info_collect
    )
    config = Config()
...
def chat_multi_model(retriever, no_model):
    ...
    info_collect = {}
    model_request = ModelRequestService(model_configurations)
    model_request.process_request_multi_model(
        question=question,
        retriever=retriever,
...
def add_additional_feedback_feedback(feedback_id):
    data = request.get_json()
    update = feedback_collection.update_one(
        {"_id": ObjectId(feedback_id)}, {"$push": {"additional_feedback": data}}
    )
